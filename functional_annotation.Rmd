...
The data needed for this exercise is taken from [a similar course](http://scilifelab.github.io/courses/ngsintro/1609/labs/rnaseq_tutorial_in_progress), in particular it comes from [this publication](http://www.nature.com/neuro/journal/v19/n7/abs/nn.4316.html). We will work with the output from the differential expression analysis of KO vs WT. The data files should be in a data/ subdirectory in your R working directory. You can download the data/ directory with files [here](data.zip).
...
> **Question `r qn<-qn+1;qn`:** Could gene overlap between you gene-sets bias the result interpretation in any way?  
> **Question `r qn<-qn+1;qn`:** What is the top GO-term affected by down-regulation and up-regulation, respectively?
...
```{r, message=F, warning=F, results='hide', echo=F, fig.height=12}
...
> **Question `r qn<-qn+1;qn`:** Are these results in line with previous results we have seen in this exercise? Do you feel there is a consensus pattern?
...
`r sn<-sn+1; paste(sn,". ", sep="")` Let's do so for a specific gene-set:
...
> **Question `r qn<-qn+1;qn`:** Does the table look like you expect it to?
...
fixa toc float?
kolla subset av frågor som ska lämnas in? Lämna in kod?
